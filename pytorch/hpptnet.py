...
class CQTSpectrogram(nn.Module):
    def __init__(self, sr, hop_length, fmin, n_bins, bins_per_octave, n_fft=None, log_scale=False):
        super(CQTSpectrogram, self).__init__()
        self.cqt = nnAudio.features.cqt.CQT(sr=sr, hop_length=hop_length, fmin=fmin, 
                            n_bins=n_bins, bins_per_octave=bins_per_octave, center=True)
        self.amplitude_to_db = torchaudio.transforms.AmplitudeToDB(top_db=80)
        self.log_scale = log_scale
        logging.info('='*20 + f'log_scale = {log_scale}' + '='*20)
        self.counter = 0
        
    def forward(self, audio):
        # cqt = torch.clamp(self.cqt(audio), min=1e-7)
        cqt = self.cqt(audio)
        cqt = self.amplitude_to_db(cqt)
        if self.log_scale:
          cqt = torch.log(cqt)
        # cqt = torch.clamp(cqt, min=-8)
        # import matplotlib.pyplot as plt
        # # print(cqt.shape)
        # plt.gca().invert_yaxis()
        # plt.matshow(cqt.cpu()[0][:,:250])  # 矩阵可视化
        # plt.gca().invert_yaxis()
        # png_name = f"workspaces/cqt_png/{self.counter}.png"
        # if self.counter == 10:
        #   plt.savefig(png_name)
        #   print('saved at:\t' + png_name)
        #   exit(1)
...
class Mlp(nn.Module):
  def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
    ...
    self.act = act_layer() # nn.GELU
    # down projection 向下投影
    self.fc2 = nn.Linear(hidden_features, out_features)
    self.drop = nn.Dropout(drop) # nn.Dropout(0)

  def forward(self, x):
    x = self.fc1(x)
    x = self.act(x) # nn.GELU
    x = self.drop(x) # nn.Dropout(0)
    x = self.fc2(x)
    x = self.drop(x) # nn.Dropout(0)
    return x
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, freq_attn_mask=None, qkv_bias=False, 
      ...
      if freq_attn_mask is not None:
        ...
        self.register_buffer('freq_attn_mask', freq_attn_mask) 
        if self.apply_sparse_attn:
          # 返回一个元组，其中包含每个非零元素的索引的张量  所有False取反,即0->True
    ...
    def _apply_mask_attn(self, q, k, v):
      ...
      if hasattr(self, 'freq_attn_mask'):
        ...
        # attn.masked_fill_(self.freq_attn_mask.bool(), mask_value) # true -> -inf
        attn += self.freq_attn_mask
        # t2 = datetime.now()
        # print(f'_apply_mask_attn freq_attn_mask: {self.freq_attn_mask}')
        # print(self.freq_attn_mask[1, :5], end='\r')
      ...
      # t4 = datetime.now()
      # dt = t4 - t0
      # print(f'attn:{(t1-t0)/dt*100:.2f}%\tmask:{(t2-t1)/dt*100:.2f}%\tdrop:{(t3-t2)/dt*100:.2f}%\tx:{(t4-t3)/dt*100:.2f}%\t{self.freq_attn_mask.shape}')
      return x
...
class Transformer(nn.Module):
  def __init__(self, dim, num_heads, freq_attn_mask=None, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
    ...
    self.attn = Attention(dim, num_heads, freq_attn_mask, qkv_bias, qk_scale, attn_drop, drop)
...
# ConvTrans.png 每一层在时间维度使用带空洞的TCN和频率维度的Transformer
class ConvTrans(nn.Module):
  # ConvTrans(convtrans_channel=128, dilation=[1,1,2,2], num_heads=1, freq_attn_mask)
  def __init__(self, in_channel, dilation, num_heads, freq_attn_mask=None, out_channel=None, stride=1):
    ...
    self.stride = stride
    # 352=>88
    self.trans = Transformer(out_channel, num_heads, freq_attn_mask) 

  # ConvTrans: input: [6, 128, 352, 296]  after conv: [1776, 352, 128]
  def forward(self, x):
    print(f'\nConvTrans: input: {x.shape}', end='\t') # [6, 128, 352, 296]
    if self.stride == 1:
      # x = [1, 16, 352, 61]
      b, c, f, t = x.shape
      # -> [352, 16, 61]
    ...
    # -> [1, 128, 352, 61]
    print(f'after_conv: {x.shape}', end='\t') # [1776, 352, 128]
    out = self.trans(x).reshape(-1, t, f, c).permute(0, 3, 2, 1)
    print(f'after_trans: {out.shape}')
    return out
...
# "conv_channels": [1, 16, 16], "convtrans_channel": 128, "output_channel": 256, "dilations": [1, 1, 2, 2], "num_heads": 1
# "ignore_offset_head": false, "ignore_velocity_head": false,
      # "cqt_config":{
      #   "sr": 16000,
      #   "hop_length": 320,
      #   "n_fft": 2048,
    ...
    def _make_freq_attn_mask(self, cqt_config, max_ratio=9):
      # 计算mask矩阵，哪些地方不关注
      mask = torch.ones(cqt_config['n_bins'], cqt_config['n_bins']).bool() # "n_bins": 352,
      for n_bin in range(cqt_config['n_bins']):
        for ratio in range(1, max_ratio + 1):
          bin_distance = int(0.5 + cqt_config['bins_per_octave'] * math.log2(ratio)) # "bins_per_octave": 48
          # 0 48 76 96 111 124 135 144 152->4 * 12、19、24、28、31、34、36
    ...
    def forward(self, audio):
      ...
      # x -ConvTrans-> onset
      print(f'onset: {self.onset(x).shape}')
      onset = self.onset_pool(self.onset(x)).flatten(1)
      print(f'onset after pool: {onset.shape}')
      frame = self.frame_pool(self.frame(x)).flatten(1)

      # head即nn.Linear(output_channel=256, num_classes=88)
      if self.shared_head:
        onset_logits = self.head(onset)
        print(f'onset_logits: {onset_logits.shape}')
...
if __name__ == '__main__':
  ...
  def prepare_input(shapes):
    ...
    x /= (x.max() + 1e-7)
    # x = torch.randn(1, 1, 352, 45)
    x = x.to(device)
    return dict(audio = x)
  # 0.248s n_fft = 2048 hop_length = 320 对应7帧 Flops: 9.64 GMac Params: 6.11 M
  # ignore offset 4.56M
