...
class SegmentEvaluator(object):
    ...
    def evaluate(self, dataloader):
        ...
        if 'velocity_output' in output_dict.keys():
            ...
            if output_dict['velocity_output'].shape[-1] == config.velocity_scale:
                output_dict['velocity_output'] = output_dict['velocity_output'].argmax(-1) / config.velocity_scale
            statistics['velocity_mae'] = mae(output_dict['velocity_output'], 
                output_dict['velocity_roll'] / config.velocity_scale, output_dict['onset_roll'])
        ...
        statistics['sum'] = np.around(statistics['frame_ap'] - (statistics['reg_onset_mae'] + statistics['reg_offset_mae'] + statistics['velocity_mae'])/3, decimals=4)
