...
from utilities import (create_folder, int16_to_float32, read_metadata, traverse_folder, 
...
class MaestroDataset(object):
    def __init__(self, hdf5s_dir, segment_seconds, frames_per_second, 
        max_note_shift=0, augmentor=None, pre_spec=False):
    ...
    def __getitem__pre_spec(self, meta):
        ...
        with h5py.File(hdf5_path, 'r') as hf:
            ...
            if end_frame >= hf['pre_spec'].shape[2]:
                end_frame = start_frame
                start_frame -= int(self.segment_seconds * self.frames_per_second)
                start_time -= self.segment_seconds
            data_dict['waveform'] = hf['pre_spec'][:,:,start_frame:end_frame+1]
            data_dict['name'], _ = os.path.splitext(os.path.basename(hdf5_name))
    ...
    def __getitem__(self, meta):
        ...
        """
        if self.pre_spec:
            return self.__getitem__pre_spec(meta)
        [year, hdf5_name, start_time] = meta
        hdf5_path = os.path.join(self.hdf5s_dir, year, hdf5_name)
    ...
    def __getitem__1(self, meta):  # use hf_cache for mini dataset
        """Prepare input and target of a segment for training.
        ...
        # Load hdf5
        if hdf5_path not in self.hf_cache:
            with h5py.File(hdf5_path, 'r') as hf:
                self.hf_cache[hdf5_path] = {}
                self.hf_cache[hdf5_path]['waveform'] = hf['waveform'][:] #int16_to_float32(hf['waveform'][:])
                self.hf_cache[hdf5_path]['midi_events'] = [e.decode() for e in hf['midi_event'][:]]
                self.hf_cache[hdf5_path]['midi_events_time'] = hf['midi_event_time'][:]
                
        start_sample = int(start_time * self.sample_rate)
        end_sample = start_sample + self.segment_samples

        hf = self.hf_cache[hdf5_path]
        if end_sample >= hf['waveform'].shape[0]:
            start_sample -= self.segment_samples
            end_sample -= self.segment_samples

        waveform = int16_to_float32(hf['waveform'][start_sample : end_sample])
        # waveform = self.hf_cache[hdf5_path]['waveform'][start_sample : end_sample] 
        ...
        # midi_events = [e.decode() for e in hf['midi_event'][:]]
        # midi_events_time = hf['midi_event_time'][:]
        midi_events = self.hf_cache[hdf5_path]['midi_events'] 
        midi_events_time = self.hf_cache[hdf5_path]['midi_events_time'] 
...
class OMAP_Dataset(object):
    def __init__(self, hdf5s_dir, segment_seconds, frames_per_second, 
        ...
        self.max_note_shift = max_note_shift
        self.begin_note = config.begin_note
        self.classes_num = config.classes_num # 88
        self.segment_samples = int(self.sample_rate * self.segment_seconds)
        self.augmentor = augmentor
    ...
    def __getitem__(self, meta):
        ...
        with h5py.File(hdf5_path, 'r') as hf:
            ...
            #         'offset_time': note[1], 
            #         'velocity': int(note[3])})
            midi_dict = midi_events 
            # Process MIDI events to target
            (target_dict, note_events) = \
                self.target_processor.process_note_events(start_time, midi_dict, note_shift=note_shift)
...
class Sampler(object):
    def __init__(self, hdf5s_dir, split, segment_seconds, hop_seconds, 
            batch_size, mini_data, random_seed=1234, dataset='omap'):
        ...
        assert dataset in ['maestro', 'maestro_slim', 'maestro_v3_mel', 'maestro_v3_cqt', 'maestro_v3_cqt', 'maps', 'omap']
        self.hdf5s_dir = hdf5s_dir
        self.segment_seconds = segment_seconds
        ...
        self.segment_list = []
        meta_dict = read_metadata(os.path.join(hdf5s_dir, 'meta_dict.csv'))

        # n = 0
        # for hdf5_path in hdf5_paths:
        #     with h5py.File(hdf5_path, 'r') as hf:
        for n in range(len(meta_dict['canonical_composer'])):
            if meta_dict['split'][n] == split:
                    audio_name = '{}.h5'.format(os.path.splitext(meta_dict['audio_filename'][n].split('/')[-1])[0])
                    year = meta_dict['year'][n]
                    duration = meta_dict['duration'][n]
                # if hf.attrs['split'].decode() == split:
                    # audio_name = hdf5_path.split('/')[-1]
                    # year = hf.attrs['year'].decode() if 'maestro' in dataset  else 0
    ...
    def load_state_dict(self, state):
        if state['segment_indexes'].shape == self.segment_indexes.shape:
...
class TestSampler(object):
    def __init__(self, hdf5s_dir, split, segment_seconds, hop_seconds, 
        ...
        Args:
          ...
          mini_data: bool, sample from a small amount of data for debugging
        """
        assert split in ['train', 'valid', 'test', 'validation']
        assert dataset in ['maestro', 'maestro_slim', 'maestro_v3_mel', 'maestro_v3_cqt', 'maps', 'omap']
        self.hdf5s_dir = hdf5s_dir
        self.segment_seconds = segment_seconds
        self.hop_seconds = hop_seconds
        self.sample_rate = config.sample_rate
